...
def Question5():
    ...
    # Try every possible state of the 17-bit LFSR
    ...
    for initial_state_17 in range(premier_etat, dernier_etat): #on teste tous les états possibles du premier LFSR
        ...
        for i in range(6):
            if(z[i] - x[i] > 0):
                ...
                c = 1
                y.append(z[i] - x[i] - c + 256)
        
        test_seed = 1 << 24 | y[0] << 16 | y[1] << 8 | y[2]

        test_bytes = []

        test_lfsr25 = LFSR(test_seed, coeffs_LFSR_25)
        ...
        if correspondance:
            found_s = x[0][0] << 32 | x[1][0] << 24 | y[0] << 16 | y[1] << 8 | y[0]
            break
